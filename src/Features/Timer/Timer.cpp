...
Variable p2sm_timer_always_running("p2sm_timer_always_running", "1", "Timer will save current value when disconnecting.\n");
Variable p2sm_timer_time_pauses("p2sm_timer_time_pauses", "1", "Timer adds non-simulated ticks when server pauses.\n");
...
CON_COMMAND(p2sm_timer_start, "p2sm_timer_start - starts timer\n") {
	if (timer->isRunning) {
	...
	if (p2sm_stats_auto_reset.GetInt() >= 2) {
		stats->ResetAll();
	}
}
CON_COMMAND(p2sm_timer_stop, "p2sm_timer_stop - stops timer\n") {
	if (!timer->isRunning) {
...
CON_COMMAND(p2sm_timer_result, "p2sm_timer_result - prints result of timer\n") {
	auto tick = timer->GetTick(engine->GetTick());
...
CON_COMMAND(p2sm_avg_start, "p2sm_avg_start - starts calculating the average when using timer\n") {
	timer->avg->Start();
}
CON_COMMAND(p2sm_avg_stop, "p2sm_avg_stop - stops average calculation\n") {
	timer->avg->isEnabled = false;
}
CON_COMMAND(p2sm_avg_result, "p2sm_avg_result - prints result of average\n") {
...
CON_COMMAND(p2sm_cps_add, "p2sm_cps_add - saves current time of timer\n") {
	if (!timer->isRunning) {
		return console->DevMsg("Timer isn't running!\n");
	}
...
CON_COMMAND(p2sm_cps_clear, "p2sm_cps_clear - resets saved times of timer\n") {
	timer->cps->Reset();
}
CON_COMMAND(p2sm_cps_result, "p2sm_cps_result - prints result of timer checkpoints\n") {
...
ON_EVENT(PRE_TICK) {
	...
	if (session->isRunning && pauseTimer->IsActive() && timer->isRunning && p2sm_timer_time_pauses.GetBool()) {
		++timer->totalTicks;
	}
}
