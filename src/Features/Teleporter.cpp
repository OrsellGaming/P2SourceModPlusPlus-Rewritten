...
void Teleporter::TeleportLocal(int slot, bool portals) {
	...
	auto &location = this->GetLocation(slot);
	if (!location.isSet) {
		const char msg[] = "Location not set! Use p2sm_teleport_setpos.";
...
CON_COMMAND(p2sm_teleport, "p2sm_teleport [noportals] - teleports the player to the last saved location\n") {
	IGNORE_DEMO_PLAYER();
...
CON_COMMAND(p2sm_teleport_setpos, "p2sm_teleport_setpos - saves current location for teleportation\n") {
