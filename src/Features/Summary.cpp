...
Variable p2sm_sum_during_session("p2sm_sum_during_session", "1", "Updates the summary counter automatically during a session.\n");
...
CON_COMMAND(p2sm_sum_here, "p2sm_sum_here - starts counting total ticks of sessions\n") {
...
CON_COMMAND(p2sm_sum_stop, "p2sm_sum_stop - stops summary counter\n") {
	if (!summary->isRunning) {
		return console->Print("There's no summary to stop!\n");
	}

	if (p2sm_sum_during_session.GetBool()) {
...
CON_COMMAND(p2sm_sum_result, "p2sm_sum_result - prints result of summary\n") {
