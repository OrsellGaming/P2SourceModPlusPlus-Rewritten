...
Variable p2sm_stats_jumps_xy("p2sm_stats_jumps_xy", "0", "Saves jump distance as 2D vector.\n");
Variable p2sm_stats_velocity_peak_xy("p2sm_stats_velocity_peak_xy", "0", "Saves velocity peak as 2D vector.\n");
Variable p2sm_stats_auto_reset("p2sm_stats_auto_reset", "0", 0,
                              "Resets all stats automatically.\n"
                              "0 = Default,\n"
                              "1 = Restart or disconnect only,\n"
                              "2 = Any load & p2sm_timer_start.\n"
...
CON_COMMAND(p2sm_stats_jump, "p2sm_stats_jump - prints jump stats\n") {
...
CON_COMMAND(p2sm_stats_steps, "p2sm_stats_steps - prints total amount of steps\n") {
	auto stat = stats->Get(GET_SLOT());
	console->Print("Steps: %i\n", stat->jumps->total);
}
CON_COMMAND(p2sm_stats_velocity, "p2sm_stats_velocity - prints velocity stats\n") {
	auto nSlot = GET_SLOT();
...
CON_COMMAND(p2sm_stats_jumps_reset, "p2sm_stats_jumps_reset - resets total jump count and jump distance peak\n") {
	stats->Get(GET_SLOT())->jumps->Reset();
}
CON_COMMAND(p2sm_stats_steps_reset, "p2sm_stats_steps_reset - resets total step count\n") {
	stats->Get(GET_SLOT())->steps->Reset();
}
CON_COMMAND(p2sm_stats_velocity_reset, "p2sm_stats_velocity_reset - resets velocity peak\n") {
	stats->Get(GET_SLOT())->velocity->Reset();
}
CON_COMMAND(p2sm_stats_reset, "p2sm_stats_reset - resets all saved stats\n") {
...
CON_COMMAND(p2sm_export_stats, "p2sm_export_stats <filepath> -  export the stats to the specified path in a .csv file\n") {
	bool result = false;
	std::string path = args.ArgC() == 1 ? p2sm_statcounter_filePath.GetString() : args[1];

	if (args.ArgC() >= 1) {
		result = statsCounter->ExportToFile(path);
	} else {
		return console->Print(p2sm_export_stats.ThisPtr()->m_pszHelpString);
	}
...
CON_COMMAND(p2sm_import_stats, "p2sm_import_stats <filePath> - import the stats from the specified .csv file\n") {
	bool result = false;
	std::string path = args.ArgC() == 1 ? p2sm_statcounter_filePath.GetString() : args[1];
	...
	if (args.ArgC() >= 1) {
		...
		return console->Print(p2sm_import_stats.ThisPtr()->m_pszHelpString);
	}
...
CON_COMMAND(p2sm_print_stats, "p2sm_print_stats - prints your statistics if those are loaded\n") {
	statsCounter->Print();
}
