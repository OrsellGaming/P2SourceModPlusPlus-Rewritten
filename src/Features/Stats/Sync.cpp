...
Variable p2sm_strafesync("p2sm_strafesync", "0", "Shows strafe sync stats.\n");
Variable p2sm_strafesync_session_time("p2sm_strafesync_session_time", "0", 0,
                                     "In seconds. How much time should pass until session is reset.\n"
                                     "If 0, you'll have to reset the session manually.\n");
Variable p2sm_strafesync_noground("p2sm_strafesync_noground", "1",
                                 "0: Always run.\n"
...
void Sync::UpdateSync(int slot, const CUserCmd *cmd) {
	...
	if (p2sm_strafesync_noground.GetBool()) {
		if (player->ground_entity()) {
			return;
		}
	}

	if (p2sm_strafesync_session_time.GetInt() > 0) {
		if (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - this->start).count() >= p2sm_strafesync_session_time.GetInt()) {
...
CON_COMMAND(p2sm_strafesync_pause, "p2sm_strafesync_pause - pause strafe sync session\n") {
	synchro->PauseSyncSession();
}

CON_COMMAND(p2sm_strafesync_resume, "p2sm_strafesync_resume - resume strafe sync session\n") {
	synchro->ResumeSyncSession();
}

CON_COMMAND(p2sm_strafesync_reset, "p2sm_strafesync_reset - reset strafe sync session\n") {
	synchro->ResetSyncSession();
}

CON_COMMAND(p2sm_strafesync_split, "p2sm_strafesync_split - makes a new split\n") {
