...
CON_COMMAND_F(p2sm_echo, "p2sm_echo <color> <string...> - echo a string to console with a given color\n", FCVAR_DONTRECORD) {
	if (args.ArgC() < 2) {
		return console->Print(p2sm_echo.ThisPtr()->m_pszHelpString);
	}
...
CON_COMMAND_F(p2sm_echo_nolf, "p2sm_echo_nolf <color> <string...> - echo a string to console with a given color and no trailing line feed\n", FCVAR_DONTRECORD) {
	if (args.ArgC() < 2) {
		return console->Print(p2sm_echo_nolf.ThisPtr()->m_pszHelpString);
	}
